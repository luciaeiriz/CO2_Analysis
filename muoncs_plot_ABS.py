...
def plot_scan(scan, wp):
    ...
    n_events = len(muon_cs)
    muon_cs_err_adjusted = muon_cs_err / np.sqrt(n_events)
    ...
    label = '\n'.join(labels)
    plt.errorbar(HV_adj, muon_cs, yerr=muon_cs_err_adjusted, fmt=marker, markersize=9, color=color, label=label)
    plt.plot(HV_adj, muon_cs, color=color)
...
plt.ylabel('Muon cluster size (nº of strips)', fontsize=38)
plt.grid(ls='--')
...
plt.xlim(-800, 550)
plt.ylim(0.8, 4)
plt.plot([0, 0], [0, 2.5], linestyle='--', color='gray')
...
# Plotting Background rate: {stdmx_background_rate:.2f} kHz/cm²
label = f"No background rate \nAfter irradiation \nThreshold: 60 fC \n1.4 mm double gap RPC" 
...
plt.savefig(os.path.join('Plots_2024/muoncs_' + name + ".png"))
#plt.show()
